...
int main(){
    ...
    cout<<"This is an application which aims to solve imo 2011 windmill problem(q2)"<<endl<<"user can either go for randomised non collinear points generator or can input non collinear points manually or you can go on full custimize mode in which you can select the initial slope and centre of rotation point for line or there is a testcase mode in which custom testcase is there"<<endl<<"enter [1] for randomised points generator"<<endl<<"enter [2] for manually entering points"<<endl<<"enter [3] for entering full customizable mode(line customization)"<<endl<<"enter [4] for seeing custom testcase"<<endl;
    cin>>user_input;
    if(user_input==3){
    ...
    bool colinearity_check=colinearity_checker(vec);
    if(colinearity_check==false){
        return 0;
    }
    ...
    for(vector<double> point:line_path){
        ...
        if((point[0]==vec_to_double[special_index][0])&(point[1]==vec_to_double[special_index][1])){
            ...
            store_local_count=local_count;
            // if(inside_flag==true){
            //     if((point[0]==vec_to_double[next_special_index][0])&(point[1]==vec_to_double[next_special_index+1][1])){
        ...
        if(local_count-store_local_count==1){
            if((point[0]==vec_to_double[next_special_index][0])&(point[1]==vec_to_double[next_special_index][1])&(local_count>2)){
                // cout<<point[0]<<" "<<point[1]<<endl;
                cycle_length=local_count-2;
                whether_reached=true;
            }
        }
